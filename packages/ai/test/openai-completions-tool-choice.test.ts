...
describe("openai-completions tool_choice", () => {
	...
	it("preserves z.ai thinking when replaying reasoning_content", async () => {
		...
		await streamSimple(
			...
			{
				messages: [
					{ role: "user", content: "Read README-PI.md", timestamp: Date.now() },
					assistantMessage,
					toolResult,
	...
	it("ignores empty custom objects on function tool call deltas", async () => {
							...
							tool_calls: [
								{
									...
									id: "call_1",
									type: "function",
									function: { name: "read", arguments: '{"path":"README-PI.md"}' },
									custom: {},
								},
							],
						},
		...
		const response = await streamSimple(
			model,
			{
				messages: [{ role: "user", content: "Read README-PI.md", timestamp: Date.now() }],
				tools: [tool],
			},
			{ apiKey: "test" },
		).result();
